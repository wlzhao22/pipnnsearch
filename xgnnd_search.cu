...
#include <vector>
#include <mutex>
static std::mutex g_xgnnd_index_mtx;
static IndexLoad g_xgnnd_shared_index;
static bool g_xgnnd_index_loaded = false;
static int  g_xgnnd_index_refcnt = 0;
...
#define GRAPH_INDEX_METADATA_FILE_SUFFIX "_disk_metadata.bin" // This is explicitly generated by running XGNND script xgnnd_preprocess.py
...
template <typename T>
XGNNDSearch<T>::XGNNDSearch()
{
	m_pImpl = new XGNNDSearchInner<int>();
}

template<typename T>
XGNNDSearch<T>::~XGNNDSearch()
{
	XGNNDSearchInner<T>* pobjBangInner = static_cast<XGNNDSearchInner<T>*>(m_pImpl);
	delete pobjBangInner;
}

template<typename T>
bool XGNNDSearch<T>::xgnnd_load( char* indexfile_path_prefix)
{
	XGNNDSearchInner<T>* pobjBangInner = static_cast<XGNNDSearchInner<T>*>(m_pImpl);
	return pobjBangInner->xgnnd_load(indexfile_path_prefix);
}

template<typename T>
void XGNNDSearch<T>::xgnnd_alloc(int numQueries)
{
	XGNNDSearchInner<T>* pobjBangInner = static_cast<XGNNDSearchInner<T>*>(m_pImpl);
	pobjBangInner->xgnnd_alloc(numQueries);
}

template<typename T>
void XGNNDSearch<T>::xgnnd_init(int numQueries)
{
	XGNNDSearchInner<T>* pobjBangInner = static_cast<XGNNDSearchInner<T>*>(m_pImpl);
	pobjBangInner->xgnnd_init(numQueries);
}

template<typename T>
void XGNNDSearch<T>::xgnnd_set_searchparams(int recall,
                            int worklist_length,
                            DistFunc nDistFunc)
{
	XGNNDSearchInner<T>* pobjBangInner = static_cast<XGNNDSearchInner<T>*>(m_pImpl);
	return pobjBangInner->xgnnd_set_searchparams(recall, worklist_length, nDistFunc);
}

template<typename T>
void XGNNDSearch<T>::xgnnd_query(T* query_array,
                    int num_queries,
                    result_ann_t* nearestNeighbours,
					float* nearestNeighbours_dist )
{
	XGNNDSearchInner<T>* pobjBangInner = static_cast<XGNNDSearchInner<T>*>(m_pImpl);
	pobjBangInner->xgnnd_query(query_array, num_queries, nearestNeighbours, nearestNeighbours_dist);
}

template<typename T>
void XGNNDSearch<T>::xgnnd_free()
{
	XGNNDSearchInner<T>* pobjBangInner = static_cast<XGNNDSearchInner<T>*>(m_pImpl);
	pobjBangInner->xgnnd_free();
}

template<typename T>
void XGNNDSearch<T>::xgnnd_unload()
{
	XGNNDSearchInner<T>* pobjBangInner = static_cast<XGNNDSearchInner<T>*>(m_pImpl);
	pobjBangInner->xgnnd_unload();
}


template<typename T>
bool XGNNDSearchInner<T>::xgnnd_load(char* indexfile_path_prefix)
{

	std::lock_guard<std::mutex> lk(g_xgnnd_index_mtx);

	if (g_xgnnd_index_loaded) {
		m_objInputData = g_xgnnd_shared_index; 
		g_xgnnd_index_refcnt++;
		return true;
	}
    ...
    m_objInputData.pIndex = (uint8_t*)mmap(NULL, m_objInputData.size_indexfile,
                                           PROT_READ | PROT_WRITE,
                                           MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB | MAP_POPULATE,
                                           -1, 0);
    if (m_objInputData.pIndex == MAP_FAILED) {
        printf("[Warning] Huge Pages allocation failed (MAP_FAILED). Falling back to standard malloc.\n");
		return false;
        m_objInputData.pIndex = (uint8_t*)malloc(m_objInputData.size_indexfile);
    } else {
        printf("[Info] Graph Index allocated successfully using Huge Pages.\n");
    }
#else
    m_objInputData.pIndex = (uint8_t*)malloc(m_objInputData.size_indexfile);
#endif
...
if (bRet) {
    g_xgnnd_shared_index = m_objInputData;
    g_xgnnd_index_loaded = true;
    g_xgnnd_index_refcnt = 1;
}
return bRet;
	// return bRet;
}
...
template<typename T>
void XGNNDSearchInner<T>::xgnnd_alloc(int numQueries)
{
	...
	unsigned threads = 64;
	const char* env = getenv("XGNND_CPU_THREADS");
	if (env) {
		int v = atoi(env);
		if (v > 0) threads = (unsigned)v;
	}
	m_objHostInst.numCPUthreads = threads;
	std::cout << "m_objHostInst.numCPUthreads : " << m_objHostInst.numCPUthreads << std::endl;
...
template<typename T>
void XGNNDSearchInner<T>::xgnnd_init(int numQueries)
{
...
template<typename T>
void XGNNDSearchInner<T>::xgnnd_free()
{
...
template<typename T>
void XGNNDSearchInner<T>::xgnnd_unload()
{
    std::lock_guard<std::mutex> lk(g_xgnnd_index_mtx);

    if (!g_xgnnd_index_loaded) return;

    g_xgnnd_index_refcnt--;
    if (g_xgnnd_index_refcnt > 0) return;

    printf("Bang Unload (shared)\n");

#ifdef _HUGEPAGES
    // 如果开启了大页，使用 munmap 释放
    // 注意：需要包含 <sys/mman.h>
    munmap(g_xgnnd_shared_index.pIndex, g_xgnnd_shared_index.size_indexfile);
    printf("Index unloaded via munmap (Huge Pages)\n");
#else
    // 默认情况，使用 free
    free(g_xgnnd_shared_index.pIndex);
#endif

	gpuErrchk(cudaFree(g_xgnnd_shared_index.d_compressedVectors));
    gpuErrchk(cudaFree(g_xgnnd_shared_index.d_pqTable));
    gpuErrchk(cudaFree(g_xgnnd_shared_index.d_chunksOffset));
    gpuErrchk(cudaFree(g_xgnnd_shared_index.d_centroid));

    memset(&g_xgnnd_shared_index, 0, sizeof(IndexLoad));
    g_xgnnd_index_loaded = false;
    g_xgnnd_index_refcnt = 0;
}

template<typename T>
void XGNNDSearchInner<T>::xgnnd_set_searchparams(int recall, int worklist_length, DistFunc nDistFunc)
{
...
template<typename T>
void XGNNDSearchInner<T>::xgnnd_query(T* queriesFP, int numQueries,
					result_ann_t* nearestNeighbours,
					float* nearestNeighbours_dist )
{
	...
	do
	{
		...
		start = std::chrono::high_resolution_clock::now();
#endif
		unsigned offset =  (m_objInputData.R+1); 
		...
		#pragma omp parallel
		{
			...
			// 局部引用，减少反复解引用成员变量
			const uint64_t entryLen = m_objInputData.ullIndex_Entry_LEN;
			uint8_t* const pIndex   = m_objInputData.pIndex;

			const unsigned D = (unsigned)m_objInputData.D;
			const size_t fpBytes = (size_t)D * sizeof(T);

			const int QBLK = 128;   
			const int PF_LINES = 8;  

			uint8_t* src_list[QBLK];
			int      q_list[QBLK];
			int      cnt = 0;

			for (int base = tid * QBLK; base < numQueries; base += nth * QBLK)
			{
				cnt = 0;
				...
				{
					unsigned numParents = m_objHostInst.parents[(size_t)q * SIZEPARENTLIST];
					if (numParents == 0) continue;

					unsigned parent = m_objHostInst.parents[(size_t)q * SIZEPARENTLIST + 1];
					uint8_t* src = pIndex + entryLen * (uint64_t)parent;
					...
					for (int l = 0; l < PF_LINES; ++l) {
						__builtin_prefetch(src + (size_t)l * 64, 0, 3);
					}
				}

				const unsigned offset = (m_objInputData.R + 1);

				for (int k = 0; k < cnt; ++k)
				{
					const int q = q_list[k];
					uint8_t* src = src_list[k];

					const unsigned offset_neighbors = (unsigned)q * offset;

					memcpy(FPSetCoordsList + (iter * m_objHostInst.FPSetCoords_rowsize) + ((size_t)q * m_objHostInst.FPSetCoords_size),
						src,
						fpBytes);

					unsigned* puNumNeighbours = (unsigned*)(src + fpBytes);
					unsigned deg = *puNumNeighbours;  

					if (deg > m_objInputData.R) deg = m_objInputData.R;

					memcpy(m_objHostInst.neighbors + offset_neighbors,
						puNumNeighbours + 1,
						(size_t)deg * sizeof(unsigned));

					m_objHostInst.numNeighbors_query[q] = deg;
				}
			}
		}
...
#if 0
void xgnnd_set_searchparams_c(int recall, int worklist_length, DistFunc nDistFunc)
{
	m_objSearchParams.recall = recall;
	m_objSearchParams.worklist_length = worklist_length;
	m_objSearchParams.uDistFunc = nDistFunc;
...
void xgnnd_load_c(char* pszPath)
{
	// ToDo: Add other datatypes
	xgnnd_load<uint8_t>(pszPath);
}

void xgnnd_query_c(uint8_t* query_array, int num_queries,
					result_ann_t* nearestNeighbours,
					float* nearestNeighbours_dist )
{
	xgnnd_query<uint8_t>(query_array,num_queries, nearestNeighbours, nearestNeighbours_dist );
}
#endif